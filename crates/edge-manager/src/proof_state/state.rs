...
impl ProofState {
    ...
    /// Build the per-task timeline that `/proof_pipeline` serves.
...
/// Per-task timeline of one proof.
#[derive(Debug, Serialize, Clone)]
...
/// One task of the timeline.
#[derive(Debug, Default, Serialize, Clone)]
pub struct TaskTiming {
    pub worker_id: usize,
    pub completed_at_ms: u64,
    #[serde(flatten)]
    pub stamps: protocol::TaskStamps,
...
mod tests {
    ...
    fn test_pipeline_reports_worker_and_segments_per_task() {
        ...
        let app_tasks: Vec<(usize, usize)> = pipeline
            .app_proofs
            .iter()
            .map(|t| (t.segment_start, t.worker_id))
            .collect();
        assert_eq!(app_tasks, vec![(0, 5), (2, 3)]);
        ...
        assert_eq!(
            [
                &internal_json["dispatched_at_ms"],
                &internal_json["worker_start_ms"],
                &internal_json["worker_end_ms"],
                &internal_json["completed_at_ms"],
            ],
            [10, 20, 70, 77]
        );
    }
